          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-3 xs:gap-4 sm:gap-4 md:gap-5 lg:gap-6'>
            ...
            <div className='space-y-2 xs:space-y-3 sm:space-y-3 md:space-y-4'>
              ...
              <div className='flex flex-col items-start space-y-2 xs:space-y-3 sm:space-y-3'>
                <div className='flex items-center gap-1.5 xs:gap-2 sm:gap-2 md:gap-3 w-full justify-center sm:justify-start'>
                  <button
                    onClick={() => setQuantity(q => Math.max(1, q - 1))}
                    className='w-7 h-7 xs:w-8 xs:h-8 sm:w-8 sm:h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-full border border-[#C6A884] flex items-center justify-center hover:bg-[#C6A884] transition-colors flex-shrink-0'>
                ...
                <div className='bg-gray-50 p-2 xs:p-3 sm:p-3 md:p-4 rounded-lg w-full'>
                  <p className='font-bengaly text-[#4A382B] text-xs xs:text-sm sm:text-sm md:text-base flex justify-between items-center'>
                    Итого:
                    <span
              ...
              {/* Кнопка добавления в корзину - больше breakpoints */}
              <button
                onClick={handleAddToCart}
                disabled={addingToCart}
                className={`w-full px-3 xs:px-4 sm:px-4 md:px-5 lg:px-6 py-2 xs:py-2.5 sm:py-2.5 md:py-3 bg-[#4A382B] text-white rounded-md hover:bg-[#4A382B]/90 transition-colors flex items-center justify-center gap-1.5 xs:gap-2 sm:gap-2 font-bengaly text-sm xs:text-base sm:text-base md:text-lg ${
                  addingToCart ? 'opacity-50 cursor-not-allowed' : ''
                }`}>
                <ShoppingCart className='w-3.5 h-3.5 xs:w-4 xs:h-4 sm:w-4 sm:h-4 md:w-4.5 md:h-4.5 lg:w-5 lg:h-5' />
